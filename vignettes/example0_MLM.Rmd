...
This document provides a step-by-step guide to evaluating Bayesian multilevel models using the `bleval` R package. We illustrate the workflow with a simulated dataset, performing parameter estimation in JAGS and computing information criteria (DIC, WAIC, LOOIC) and marginal likelihood.
...
To use `bleval`, we first need to install the package. The following two options are provided:

1.  **Install from github**

```{r}
# Note: This package is not yet public. For now, it can only be installed locally.
# Once it's public, use the following command to install it.
devtools::install_github("luoxh3/bleval")
```

2.  **Manually install from a local `.tar.gz` file:**
